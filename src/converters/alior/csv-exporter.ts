/**
 * CSV/TXT Exporter for Alior Bank MT940 converter
 * Exports transactions to tab-separated TXT format compatible with accounting software.
 *
 * Same accounting format as PKO/BNP exporters (shared output schema).
 * Date handling: Alior stores dates as YYMMDD (same as PKO MT940).
 */
...
export class CsvExporter {
  ...
  exportAuxiliary(transactions: ProcessedTransaction[]): string {
    ...
    if (incomeTransactions.length > 0) {
      ...
      for (let i = 0; i < incomeTransactions.length; i++) {
        ...
        if (transaction.original.details.counterpartyAddress) {
          lines.push(`Adres kontrahenta: ${transaction.original.details.counterpartyAddress}`);
        }
    ...
    if (expenseTransactions.length > 0) {
      ...
      for (const transaction of unrecognizedExpenses) {
        ...
        if (transaction.original.details.counterpartyAddress) {
          lines.push(`Adres: ${transaction.original.details.counterpartyAddress}`);
        }
  ...
  /**
   * Format date from YYMMDD to accounting format.
   */
