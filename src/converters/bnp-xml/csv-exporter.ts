/**
 * CSV/TXT Exporter for BNP Paribas XML converter
 * Exports transactions to tab-separated TXT format compatible with accounting software.
 *
 * Same accounting format as Santander exporter (shared output schema).
 * Date handling differs: BNP stores dates as YYYY-MM-DD.
 */
...
export class CsvExporter {
  ...
  export(transactions: ProcessedTransaction[]): string {
    ...
    for (const transaction of recognizedIncome) {
      const aptNum = this.extractApartmentNumber(transaction)!;
      const date = this.formatDate(transaction.original.bookingDate);
      const description = this.cleanDescription(transaction.original.description);
      const amount = this.formatAmount(transaction.original.amount);
  ...
  exportAuxiliary(transactions: ProcessedTransaction[]): string {
    ...
    if (incomeTransactions.length > 0) {
      ...
      for (let i = 0; i < incomeTransactions.length; i++) {
        ...
        lines.push(`Kwota: ${amount} ${transaction.original.currency}`);
        ...
        if (transaction.original.counterpartyAddress) {
          lines.push(`Adres kontrahenta: ${transaction.original.counterpartyAddress}`);
        }
        if (transaction.original.counterpartyAccount) {
          lines.push(`Nr rachunku: ${transaction.original.counterpartyAccount}`);
        }
    ...
    if (expenseTransactions.length > 0) {
      ...
      for (const transaction of unrecognizedExpenses) {
        ...
        if (transaction.original.counterpartyAddress) {
          lines.push(`Adres: ${transaction.original.counterpartyAddress}`);
        }
  ...
  private createLine(data: {
    ...
    return [
      ...
      data.tresc, data.kwota, data.k_wn, data.k_ma,
    ].join(this.options.separator);
  }
  ...
  /**
   * Format date from BNP's YYYY-MM-DD to accounting format.
   */
  private formatDate(date: string): string {
    // BNP date is YYYY-MM-DD (e.g., "2026-02-27")
